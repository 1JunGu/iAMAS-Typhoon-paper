...
def plot_models(model_list, ax0, ax1):
    ...
    line_format_dic = {"CMA": "k-", "u3km": "r-", "v3km": "b-", "v4km": "c-", "u60km": "g-"}
    line_format_dic_steer = { "u3km": "r--", "v3km": "b--", "v4km": "c--", "u60km": "g--"}
    for model_name in model_list:
        if model_name == "CMA":
            ...
            x_hours= (model_df.index - pd.to_datetime('2021-07-21 00:00:00'))/np.timedelta64(1, 'h')
            line_format = line_format_dic[model_name]
            ax0.plot(x_hours, model_df['zonal_speed'], line_format, label='U3KM translation speed',visible=True,color = "#d02090")
            ax1.plot(x_hours, model_df['meridional_speed'], line_format, label='U3KM translation speed',visible=True,color = "#d02090")

            model_df_steer = read_steer_csv(steer_csv_path + steer_csv_prefix + model_name + ".csv")
            line_format = line_format_dic_steer[model_name]
            ax0.plot(x_hours, model_df_steer['uzonal'], line_format, label='U3KM steering flow',visible=True,color="#d02090")
            ax1.plot(x_hours, model_df_steer['umeridional'], line_format, label='U3KM steering flow',visible=True,color="#d02090")
            ...
            x_hours= (model_df.index - pd.to_datetime('2021-07-21 00:00:00'))/np.timedelta64(1, 'h')
            line_format = line_format_dic[model_name]
            ax0.plot(x_hours, model_df['zonal_speed'], line_format, label='U60KM translation speed',visible=True,color = "dimgray")
            ax1.plot(x_hours, model_df['meridional_speed'], line_format, label='U60KM steering flow',visible=True,color = "dimgray")

            model_df_steer = read_steer_csv(steer_csv_path + steer_csv_prefix + model_name + ".csv")
            line_format = line_format_dic_steer[model_name]
            ax0.plot(x_hours, model_df_steer['uzonal'], line_format, label='U60KM translation speed',visible=True,color="dimgray")
            ax1.plot(x_hours, model_df_steer['umeridional'], line_format, label='U60KM steering flow',visible=True,color="dimgray")
        else:
